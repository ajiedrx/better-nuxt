    ...
    <v-row id="firstRow">
      ...
      <v-col>
        <v-card light hover flat style="border-radius: 10px">
          ...
          >
          <v-card-text>Average productivity ratio</v-card-text>
        </v-card>
      </v-col>
      <v-col>
        <v-card light hover flat style="border-radius: 10px">
          ...
          <v-card-text>Overall productive time</v-card-text>
        </v-card>
      </v-col>
    </v-row>
    <client-only>
...
export default {
  ...
  methods: {
    loadProductivityChart() {
      this.$axios
        .post('daily-tracking-report/history-per-user', {
          id_user: localStorage.getItem('user_id')
        })
        .then((response) => {
          this.value = response.data.data
          for (let i = 0; i < this.value.length; i++) {
            this.sum += this.value[i]
          }
          this.productivityAvg = this.sum / 7
    ...
    getAppData() {
      this.$axios
        ...
        .then((response) => {
          this.mostProductiveApp = response.data.data.app.productive[0]
          this.mostUnproductiveApp = response.data.data.app.not_productive[0]
          this.loaded = true
        })
        .catch((error) => {
          console.log(error)
